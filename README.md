...
```text
# Find your Kubernetes Master server IP using 'kubectl cluster-info' 
...
centralized admin utility to monitor Spark applications. Spark applications (drivers, executors) use a shared folder to log events. And, the Spark history server reads from 
